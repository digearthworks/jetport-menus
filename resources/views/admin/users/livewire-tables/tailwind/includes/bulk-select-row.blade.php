@if (count($bulkActions) && (($selectPage && $rows->total() > $rows->count()) || count($selected)))
    <x-admin.users.livewire-tables.table.row wire:key="row-message" class="bg-indigo-50">
        <x-admin.users.livewire-tables.table.cell :colspan="count($bulkActions) ? count($columns) + 1 : count($columns)">
    ...
    </x-admin.users.livewire-tables.table.row>
@endif
